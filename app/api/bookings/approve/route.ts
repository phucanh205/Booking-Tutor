...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const expected = String(booking?.approveTokenHash || "");
      ...
      tx.update(bookingRef, { status: "approved", approveTokenHash: null, rejectTokenHash: null });
      tx.update(slotRef, {
  ...
  <p>Yêu cầu đặt lịch của bạn đã được giáo viên duyệt.</p>
    ...
    return new NextResponse(htmlPage("Duyệt thành công", "Bạn có thể đóng tab này."), {
      status: 200,
      headers: { "content-type": "text/html; charset=utf-8" },
    });
  } catch (e) {
    return new NextResponse(htmlPage("Không thể duyệt", "Link không hợp lệ hoặc đã hết hạn."), {
      status: 403,
      headers: { "content-type": "text/html; charset=utf-8" },
    });
  }
